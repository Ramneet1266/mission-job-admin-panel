...
export default function Page() {
  ...
  const [jobData, setJobData] = useState({
    ...
    contactEmail: "",
    jobCompany: "",
    imageUrl: "", // Will be set dynamically after upload
    tags: [] as string[],
  });
  const [jobImageFile, setJobImageFile] = useState<File | null>(null); // New state for image file
  ...
  const openJobModal = (category: Category) => {
    ...
    setJobImageFile(null); // Reset file input
  ...
  const handleTagsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, "categories", selectedCategory.id, "tags"), {
        tag: tagsData.tag,
        createdAt: new Date().toISOString(),
      });
      console.log("Tag added successfully for category:", selectedCategory.id);
      closeModal();
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  const handleJobSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedCategory) return;
    try {
      let imageUrl = jobData.imageUrl; // Default to empty string unless file is uploaded

      // If an image file is selected, upload it to Firebase Storage
      if (jobImageFile) {
        const storageRef = ref(storage, `jobImages/${jobImageFile.name}`);
      ...
      await addDoc(collection(db, "categories", selectedCategory.id, "posting"), {
      ...
      console.log("Job posting added successfully for category:", selectedCategory.id);
      closeModal();
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
          ...
          <tbody>
            {filteredCategories.length > 0 ? (
              filteredCategories.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-100 transition">
                  <td className="px-4 py-3">{cat.title}</td>
                  <td className="px-4 py-3">{cat.information || "N/A"}</td>
                ...
                <div className="flex flex-wrap gap-2">
                  {categoryTags.length > 0 ? (
                    categoryTags.map((tag) => {
                      const isSelected = jobData.tags.includes(tag.tag);
                      return (
                        <button
                          type="button"
                          key={tag.id}
                    ...
                    <p className="text-sm text-gray-500">
                      No tags available for this category. Add tags first.
                    </p>
                  )}
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Click to select/deselect tags (currently selected: {jobData.tags.length})
                </p>
              </div>
              <div className="md:col-span-2">
                <button
