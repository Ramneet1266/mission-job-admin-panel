...
export default function PostingPage() {
  ...
  const openEditModal = (job: JobPosting) => {
    ...
    const jobCategory = categories.find((cat) => cat.id === job.categoryId);
        ...
        const categoryPromises = snapshot.docs.map((categoryDoc) => {
          ...
          return new Promise<void>((resolve) => {
            onSnapshot(
              ...
              (error) => {
                console.error(`Error fetching jobs for category ${categoryId}:`, error.message);
                resolve();
              }
            );
          });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const newCategoryId =
        categories.find((cat) => cat.title === category)?.id || "default-category-id";
      const oldCategoryId = editJob?.categoryId;

      let imageUrl = editJob?.imageUrl || ""; // Use existing URL for edits by default

      // If an image file is uploaded, override the URL
      if (imageFile) {
      ...
      // Require an image for new job postings
      if (!editJob && !imageUrl) throw new Error("No image uploaded");

      if (editJob) {
        if (oldCategoryId && oldCategoryId !== newCategoryId) {
          // Delete from old category
          await deleteDoc(doc(db, "categories", oldCategoryId, "posting", editJob.id));
          console.log("Job deleted from old category:", oldCategoryId, editJob.id);

          // Add to new category
          const docRef = await addDoc(collection(db, "categories", newCategoryId, "posting"), {
            jobTitle,
            jobDescription,
            salary,
            address,
          ...
          console.log("Job moved to new category with new ID:", newCategoryId, docRef.id);
        } else {
          // Update in same category
          await setDoc(doc(db, "categories", newCategoryId, "posting", editJob.id), {
            jobTitle,
            jobDescription,
            salary,
            address,
          ...
          console.log("Job updated in same category:", newCategoryId, editJob.id);
        }
      } else {
        // Add new job
        const docRef = await addDoc(collection(db, "categories", newCategoryId, "posting"), {
  ...
  const handleDelete = async (job: JobPosting) => {
    try {
      await deleteDoc(doc(db, "categories", job.categoryId, "posting", job.id));
  ...
  const filteredJobs = posting.filter((job) => {
    ...
    const matchesCategory = selectedCategory ? job.categoryId === selectedCategory.id : true;
    const matchesTags =
      selectedTags.length > 0 ? selectedTags.some((tag) => job.tags.includes(tag)) : true;
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {categoryTags.length > 0 ? (
              ...
              <p className="text-sm text-gray-500">No tags available for this category.</p>
            )}
          </div>
          <p className="text-xs text-gray-500 mt-1">
            Select up to 3 tags (currently selected: {selectedTags.length})
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          >
            ...
            <h2 className="text-3xl font-bold text-gray-800 mb-6">
              {editJob ? `Edit Job Posting: ${editJob.jobTitle}` : "Add New Job Posting"}
            </h2>
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition"
                  required={!editJob} // Required only for new postings
                />
                {editJob && (
                  <p className="text-xs text-gray-500 mt-1">
                    Current image: <a href={editJob.imageUrl} target="_blank">{editJob.imageUrl}</a>
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const selected = categories.find((cat) => cat.title === e.target.value);
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Select up to 3 tags (currently selected: {tags.length})
                </p>
              </div>
